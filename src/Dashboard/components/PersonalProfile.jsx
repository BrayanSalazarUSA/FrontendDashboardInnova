...
export const PersonalProfile = ({
  ...
  const [selectedRoleId, setSelectedRoleId] = useState(
    userProvider.rol?.rolKey
  );
  const [tempRole, setTempRole] = useState(null);

  const isEditableByClientAndAdmin =
    ...
    <main className="w-full min-h-screen py-1 md:w-2/3 lg:w-3/4">
      <div className="p-2 md:p-4">
        <div className="w-full px-6 pb-8 mt-8 sm:max-w-xl sm:rounded-lg">
          <h2 className="pl-6 text-3xl text-primary font-bold sm:text-xl">
            {name}
          </h2>
          ...
          <div className="grid max-w-2xl mx-auto mt-4">
            <div className="flex flex-col items-center space-y-5 sm:flex-row sm:space-y-0">
              <img
                className="object-cover w-40 h-40 p-1 rounded-full ring-2 ring-[#f5b73293] dark:ring-indigo-500"
                src={userImg}
                alt="Bordered avatar"
              />
              ...
              {isEditableByClientAndAdmin && (
                <div className="flex flex-col space-y-5 sm:ml-8">
                  ...
                  >
                    {t(
                      "dashboard.user-details.personal-profile.change-picture"
                    )}
                    <input
                      ref={fileInputRef}
                      type="file"
            ...
            <div className="items-center mt-8 sm:mt-14 text-[#202142]">
              <div className="flex flex-col items-center w-full mb-2 space-x-0 space-y-2 sm:flex-row sm:space-x-4 sm:space-y-0 sm:mb-6">
                <div className="w-full">
                  ...
                  >
                    {t(`dashboard.user-details.personal-profile.name`)}
                  </label>
                  <input
                    type="text"
                    id="name"
                    ...
                    placeholder={t(
                      "dashboard.user-details.personal-profile.name-placeholder"
                    )}
                    value={input.name}
              ...
              <div className="mb-2 sm:mb-6">
                <label
                  ...
                  className="block mb-2 text-sm font-medium text-primary dark:text-white"
                >
                  {t(`dashboard.user-details.personal-profile.email`)}
                </label>
                <input
                  type="email"
                  id="email"
                  className="bg-indigo-50 border border-indigo-300 text-primary text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5"
                  placeholder={t(
                    "dashboard.user-details.personal-profile.email-placeholder"
                  )}
                  value={input.email}
              ...
              <div className="mb-2 sm:mb-6">
                ...
                >
                  {" "}
                  {t(`dashboard.user-details.personal-profile.password`)}
                </label>
                <input
                  ...
                  className="bg-indigo-50 border border-indigo-300 text-primary text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5"
                  placeholder={t(
                    "dashboard.user-details.personal-profile.password-placeholder"
                  )}
                ...
                >
                  {showPasword
                    ? t("dashboard.user-details.personal-profile.hide-password")
                    : t(
                        "dashboard.user-details.personal-profile.show-password"
                      )}
                </button>
              </div>
